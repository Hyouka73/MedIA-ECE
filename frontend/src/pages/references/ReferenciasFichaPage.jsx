...
export default function ReferenciasFichaPage() {
  ...
  const handleSearchPaciente = async () => {
    if (!searchPaciente || searchPaciente.length < 3) return;
    
    try {
      setBuscando(true);
      const data = await pacientesAPI.getPacientes({ search: searchPaciente, limit: 10 });
      setPacientesEncontrados(data.data?.items || []);
    } catch (err) {
  ...
  const handleSelectPaciente = (paciente) => {
    ...
    setSearchPaciente(`${paciente.nombre} ${paciente.primer_apellido} ${paciente.segundo_apellido || ''}`.trim());
  };
      ...
      try {
        ...
        if (ref) {
          ...
          if (ref.paciente_nombre) {
            ...
            setPacienteSeleccionado({ id_paciente: ref.id_paciente, nombre: ref.paciente_nombre });
          }
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        // Si es edición, no implementamos update en este ejemplo
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ background: "#FDFAF5", border: "1px solid #DAD4CC", borderRadius: 10, padding: 20, marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              <div style={{ flex: 1 }}>
                <input
                  ...
                  placeholder="Buscar paciente por nombre, apellido o CURP..."
              ...
              <button type="button" onClick={handleSearchPaciente} disabled={buscando}
                style={{ padding: "10px 20px", background: "#2459A8", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer", fontSize: 13, fontWeight: 500, display: "flex", alignItems: "center", gap: 6 }}>
                <Search size={16} /> {buscando ? 'Buscando...' : 'Buscar'}
              </button>
            </div>
            {errors.id_paciente && <span style={{ fontSize: 10, color: "#BA2E45", marginTop: 4, display: "block" }}>⚠ {errors.id_paciente}</span>}

            {/* Resultados de búsqueda */}
            {pacientesEncontrados.length > 0 && (
              <div style={{ marginTop: 12, border: "1px solid #DAD4CC", borderRadius: 8, overflow: "hidden" }}>
                {pacientesEncontrados.map(p => (
                  <div key={p.id_paciente}
                    onClick={() => handleSelectPaciente(p)}
                    style={{ padding: "10px 14px", cursor: "pointer", borderBottom: "1px solid #DAD4CC", transition: "background 0.2s", background: pacienteSeleccionado?.id_paciente === p.id_paciente ? "#E3F2FD" : "#fff" }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "#1A1510" }}>{p.nombre} {p.primer_apellido} {p.segundo_apellido || ''}</div>
                    <div style={{ fontSize: 11, color: "#5A5048", marginTop: 2 }}>
                      Exp: {p.numero_expediente} · Edad: {p.edad} años · Sexo: {p.sexo || p.persona?.sexo}
                    </div>
                  </div>
                ))}
              </div>
            ...
            {pacienteSeleccionado && (
              <div style={{ marginTop: 12, padding: "10px 14px", background: "#E3F2FD", border: "1.5px solid #2459A8", borderRadius: 8, display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 16 }}>👤</span>
                <div>
                  <span style={{ fontSize: 13, fontWeight: 600, color: "#1A1510" }}>
                    {pacienteSeleccionado.nombre} {pacienteSeleccionado.primer_apellido}
                  </span>
                  <span style={{ fontSize: 11, color: "#5A5048", marginLeft: 8 }}>
                    Exp: {pacienteSeleccionado.numero_expediente}
                  </span>
                </div>
              </div>
            )}
          ...
          <div style={{ background: "#FDFAF5", border: "1px solid #DAD4CC", borderRadius: 10, padding: 20, marginBottom: 20 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
              ...
              <div>
                ...
                <select value={form.urgencia} onChange={(e) => updateField('urgencia', e.target.value)}
                  style={{ width: "100%", padding: "10px 12px", border: "1.5px solid #DAD4CC", borderRadius: 6, fontSize: 13, backgroundColor: "#fff" }}>
              ...
              <div>
                ...
                <select value={form.especialidad_destino} onChange={(e) => updateField('especialidad_destino', e.target.value)}
                ...
                {errors.especialidad_destino && <span style={{ fontSize: 10, color: "#BA2E45", marginTop: 4, display: "block" }}>⚠ {errors.especialidad_destino}</span>}
              </div>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <input type="text" value={form.establecimiento_destino} onChange={(e) => updateField('establecimiento_destino', e.target.value)}
                placeholder="Ej: Hospital General de Zona"
          ...
          <div style={{ background: "#FDFAF5", border: "1px solid #DAD4CC", borderRadius: 10, padding: 20, marginBottom: 20 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <textarea value={form.diagnostico_envio} onChange={(e) => updateField('diagnostico_envio', e.target.value)}
              ...
              {errors.diagnostico_envio && <span style={{ fontSize: 10, color: "#BA2E45", marginTop: 4, display: "block" }}>⚠ {errors.diagnostico_envio}</span>}
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <textarea value={form.motivo_referencia} onChange={(e) => updateField('motivo_referencia', e.target.value)}
              ...
              {errors.motivo_referencia && <span style={{ fontSize: 10, color: "#BA2E45", marginTop: 4, display: "block" }}>⚠ {errors.motivo_referencia}</span>}
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <textarea value={form.resumen_clinico} onChange={(e) => updateField('resumen_clinico', e.target.value)}
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <textarea value={form.hallazgos_relevantes} onChange={(e) => updateField('hallazgos_relevantes', e.target.value)}
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <textarea value={form.tratamiento_actual} onChange={(e) => updateField('tratamiento_actual', e.target.value)}
                placeholder="Medicamentos y dosis actuales..."
                rows={3}
                style={{ width: "100%", padding: "10px 12px", border: "1.5px solid #DAD4CC", borderRadius: 6, fontSize: 13, resize: "vertical", boxSizing: "border-box" }} />
            </div>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
            <button type="button" onClick={() => navigate(-1)}
              style={{ padding: "10px 20px", background: "transparent", border: "1.5px solid #2459A8", color: "#2459A8", borderRadius: 6, cursor: "pointer", fontSize: 13, fontWeight: 500 }}>
              Cancelar
            </button>
            <button type="submit" disabled={saving}
              style={{ padding: "10px 20px", background: "#2459A8", color: "#fff", border: "none", borderRadius: 6, cursor: saving ? "not-allowed" : "pointer", fontSize: 13, fontWeight: 500, opacity: saving ? 0.6 : 1, display: "flex", alignItems: "center", gap: 8 }}>
              <Save size={16} />
              {saving ? "Guardando..." : isEdit ? "Actualizar" : "Crear Referencia"}
            </button>
          </div>
        </form>
      </div>
