...
const NavButtons = ({
  ...
  <div className="flex justify-between pt-6 border-t border-[#DAD4CC]">
    ...
    <button
      type="button"
      onClick={onNext}
      disabled={loadingNext || disabledNext}
...
export default function NuevaConsultaPage() {
  ...
  const extraerIdEncuentro = (resp) => {
    ...
    return data?.id || data?.id_encuentro || data?.data?.id || data?.data?.id_encuentro || null
  }
  ...
  const canContinueExploracion = Boolean(encuentroId)
  ...
  const manejarPaso2 = async () => {
    if (!canStartEncounter) {
      setErrorGlobal('Para iniciar la consulta necesitas Motivo de consulta y al menos un Diagnóstico.')
      return
    }

    const principal = formData.diagnosticos[0]
    if (!principal) {
      setErrorGlobal('Debes seleccionar un diagnóstico principal.')
      return
    }
    ...
    try {
      ...
      await clinicoAPI.registrarSignos(nuevoId, {
        peso: getNumericOrNull(formData.peso, parseFloat),
        talla: getNumericOrNull(formData.talla, parseFloat),
        tension_sistolica: getNumericOrNull(formData.tension_sistolica, parseInt),
        tension_diastolica: getNumericOrNull(formData.tension_diastolica, parseInt),
        frecuencia_cardiaca: getNumericOrNull(formData.fc, parseInt),
        temperatura: getNumericOrNull(formData.temp, parseFloat),
        spo2: getNumericOrNull(formData.spo2, parseFloat),
      })
  ...
  const manejarPaso3 = async () => {
    if (!encuentroId) {
      setErrorGlobal('Primero debes iniciar la consulta.')
      return
    }
    ...
    try {
      ...
      const secundarios = formData.diagnosticos.slice(1)
      for (const d of secundarios) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F8F7F4]">
        <div className="text-center p-10">
          ...
          <button
            onClick={() => navigate(-1)}
            className="mt-4 text-[#1B4F8A] underline text-sm"
          >
            Regresar
          </button>
        </div>
    ...
    <div className="min-h-screen bg-[#F8F7F4] flex flex-col font-sans">
      <header className="bg-white border-b border-[#DAD4CC] px-8 py-4 flex justify-between items-center sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          <button
            type="button"
            onClick={() => navigate(`/expediente/${idPaciente}`)}
          ...
          {pasos.map((step, idx) => {
            ...
            return (
              <React.Fragment key={step.id}>
                <div className="flex flex-col items-center min-w-[68px]">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                      isActive
                        ? 'bg-[#1B4F8A] text-white ring-4 ring-blue-100'
                        : isCompleted
                  ...
                  <span
                    className={`text-[9px] mt-0.5 font-medium uppercase tracking-wide ${
                      isActive
                        ? 'text-[#1B4F8A]'
                        : isCompleted
                        ? 'text-[#2D8653]'
                ...
                {idx < pasos.length - 1 && (
                  <div
                    className={`w-8 h-[2px] mb-3 flex-shrink-0 ${
          ...
          {currentStep === 1 && (
            <div className="space-y-5">
              ...
              {formData.peso && formData.talla && Number(formData.talla) > 0 && (
                <div className="bg-blue-50 border border-blue-100 rounded-lg px-4 py-2 text-sm text-blue-800">
                  ...
                  {(
                    ...
                    Math.pow(parseFloat(formData.talla) / 100, 2)
                  ).toFixed(1)} kg/m²
                </div>
              )}
              ...
              <NavButtons
                ...
                disabledNext={!formData.motivo_consulta.trim()}
              />
            </div>
          )}
          ...
          {currentStep === 2 && (
            <div className="space-y-5">
              ...
              <p className="text-xs text-[#5A5048] -mt-3">
                Tu backend requiere diagnóstico desde la creación del encuentro.
              </p>
              ...
              <div className="flex flex-wrap gap-2">
                {formData.diagnosticos.map((d, index) => (
                  <div
                    key={d.id}
                    className="bg-blue-50 border border-blue-200 px-3 py-1.5 rounded-lg flex items-center gap-2"
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => eliminarDiagnostico(d.id)}
              ...
              <NavButtons
                ...
                disabledNext={!canStartEncounter}
              />
            </div>
          )}

          {currentStep === 3 && encuentroId && (
            <div className="space-y-5">
              <h2 className="text-[#1B4F8A] font-bold flex items-center gap-2 text-base">
              ...
              <NavButtons
                ...
                onNext={manejarPaso3}
                loadingNext={loading}
                labelNext="Continuar al Plan"
                disabledNext={!canContinueExploracion}
              />
            </div>
          )}
